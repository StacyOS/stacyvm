...
This is the first remote sandbox I/O slice. Non-streaming exec and buffered exec-stream calls are routed. File APIs, logs, previews, true live worker stream transport, and production-grade drain handoff remain future Phase 12 work.
...
- `internal/worker/rpc.go`: worker-side exec and exec-stream RPC handlers.
- `internal/worker/rpc_client.go`: typed exec and exec-stream RPC client methods.
- `internal/orchestrator/manager.go`: remote-owned sandbox exec and exec-stream routing.
- `internal/worker/rpc_test.go`: worker exec and exec-stream RPC handler coverage.
- `internal/worker/rpc_client_test.go`: typed exec and exec-stream client coverage.
- `internal/orchestrator/manager_test.go`: control-plane remote exec and exec-stream routing coverage.
